...
function abdominalMasculino(quantidade, idade) {
    ...
    for (let index = 19; index > 0; index--) {
        if (pontos > 10) { mapQuantidade.set(index, 10); } else
            if (pontos < 1) { mapQuantidade.set(index, 0); } else { mapQuantidade.set(index, pontos); }
        pontos -= 0.5;
    }
    return mapQuantidade.get(quantidade);
}
...
function total() {
    ...
    if (Number.isNaN(media)) document.getElementById("totalspan").innerText = 0;
    else document.getElementById("totalspan").innerText = media.toFixed(2);

    if (media < 7) {
        document.getElementById("parecerspan").setAttribute("class", "inapto");
        document.getElementById("parecerspan").innerText = "INAPTO";
    } else if (media >= 7) {
        document.getElementById("parecerspan").setAttribute("class", "apto");
        document.getElementById("parecerspan").innerText = "APTO";
    }
}
